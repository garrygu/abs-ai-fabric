...
class ABSUnifiedFramework {
    ...
    async loadAppRegistry() {
        try {
            // Try local app registry first
            console.log('🔍 Attempting to load local app registry from /static/apps-registry.json');
            const response = await fetch('/static/apps-registry.json');
            ...
            console.log(`📋 Loaded ${this.state.availableApps.length} apps from local registry:`, this.state.availableApps);
        } catch (error) {
            console.warn('⚠️ Failed to load app registry from local, trying gateway UI:', error);
            try {
                ...
                console.warn('⚠️ Failed to load app registry from gateway UI, trying fallback:', fallbackError);
                try {
                    ...
                    console.warn('⚠️ Failed to load app registry, using hardcoded apps:', finalError);
